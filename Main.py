...
def _cut(sentence):
    ...
    stop_words = set('.。；;?？》《')
    for i in sentence:
        if i in ['。', '！', '？', '?'] and len(sen) != 0 and sentence[i+1] not in stop_words:
            sen.append(i)
...
model.fit_generator(
    ...
    class_weight=[1.0, 1.0, 1.2, 1.1, 1.1, 1.3, 1.7, 1.1, 1.4, 1.4, 1.7, 1.9, 1.9],
